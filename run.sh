...
python3 emulator.py "$api_key" "$iot_ul_host" "$iot_ul_port" "$deviceId_000" "$timestamp" &
...
echo -e "Sending data to \e[93m$deviceId_001\e[0m..."
python3 emulator.py "$api_key" "$iot_ul_host" "$iot_ul_port" "$deviceId_001" "$timestamp" &
